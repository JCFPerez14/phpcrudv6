...
if (isset($_POST['upadatepassword'])) {
    ...
    if ($con->updatePassword($userId,$hashedPassword)){
        header('Location: user_accout.php?status=success');
        exit();
    }else {
        header('Location: user_accout.php?status=error');
        exit();
    }
    ...
    <div class="col-md-12">
      <div class="profile-info">
        ...
        <div class="info-body">
          ...
          <p><strong>Last Name:</strong> <?php echo $data['user_lastname'];?></p>
          <p><strong>Birthday:</strong> <?php echo $data['user_birthday'];?></p>
        </div>
      </div>
    </div>
    </div>
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (status) {
    ...
    switch (status) {
      case 'success':
        title = 'Success!';
        text = 'Record is successfully napaltan.';
        icon = 'success';
        break;
      case 'error':
        title = 'Error!';
